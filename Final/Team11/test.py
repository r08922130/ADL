...
from preprocess_grand_parent_sib_contain_middle import Preprocess as PreM
from preprocess_grand_parent_sib_97861 import Preprocess 
...
emb_model_f.load_state_dict(torch.load(f'ckpt/front_tags_97861.ckpt'))
emb_model_b.load_state_dict(torch.load(f'ckpt/back_tags_97861.ckpt'))
cls = ""
print("##### Get Embedding #####")
for mode in ['test']:
...
front_class_model.load_state_dict(torch.load(f"ckpt/gru_front_tags_97861.ckpt"))
back_class_model.load_state_dict(torch.load(f"ckpt/fcn_back_tags_97861.ckpt"))



front_Evaluation_model = Evaluation_Model(num_tags=8,ckpt_cls=f"ckpt/front_tags_97861.ckpt"\
    ,ckpt_start=f"ckpt/front_starts_97861.ckpt",\
        ckpt_end=f"ckpt/front_ends_97861.ckpt").to(device)


back_Evaluation_model = Evaluation_Model(num_tags=12,ckpt_cls=f"ckpt/back_tags_97861.ckpt"\
    ,ckpt_start=f"ckpt/back_starts_97861.ckpt",\
        ckpt_end=f"ckpt/back_ends_97861.ckpt").to(device)
middle_Evaluation_model = Evaluation_Model(num_tags=12,ckpt_cls=f"ckpt/middle_tags_97861.ckpt"\
    ,ckpt_start=f"ckpt/middle_starts_97861.ckpt",\
        ckpt_end=f"ckpt/middle_ends_97861.ckpt").to(device)
