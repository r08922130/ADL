...
from preprocess_grand_parent_sib import Preprocess as PreM
...
#print(pos_weight)
lr = 5e-6
print(tokenizer.encode(tokenizer.tokenize('令和２年２月１４日（金）１７時００分必着'),max_length=20))
...
middle_Evaluation_model = Evaluation_Model(num_tags=12,ckpt_cls=f"ckpt_{mid_ver}/back_tags_97861.ckpt"\
    ,ckpt_start=f"ckpt_{mid_ver}/back_starts_97861.ckpt",\
        ckpt_end=f"ckpt_{mid_ver}/back_ends_97861.ckpt").to(device)
...
with open(f'pred_{mode}_{ver}_{name}_th.csv', 'w', newline='') as csvFile:
