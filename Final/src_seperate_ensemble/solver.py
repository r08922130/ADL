...
class Solver:
    ...
    def train(self,train_data,val_data,model,optimizer,epochs=50,pos_weight=None,scheduler=None,part="",ver=""):
        """Data Structure
    ...
    def two_stage_train(self,train_data,val_data,model,optimizer,epochs=50,pos_weight=None,part="",ver=""):
        criterion_tags = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(pos_weight).to(self.device))
        total_loss = 0 
        min_loss = 10000
        train_len = len(train_data)
        ...
        for ep in range(epochs):
            ...
            for i, data in enumerate(train_data):
                ...
                tags = data['tags'].to(self.device)
                init_hidden = torch.zeros(2,1,768).to(self.device)
            ...
            if min_loss > total_loss/val_len:
                ...
                torch.save(model.state_dict(), f"ckpt_{ver}/cnn_{part}tags.ckpt")
    def extract_sentence_embedding(self,data,emb_model):
        emb_model.eval()
        embedding_dict = {}
        with torch.no_grad() : 
    ...
    def test(self,test_data,model,tag,test_emb=None,two_stage_model=None,threshold=None,top=3,mode='dev',tag_top=4,use_emb=False):
