...
if arg[1] == 'train':
    for name in ['dev','train']:
        ...
        if not os.path.isfile(f'processed_data/{name}.pkl'):
            ...
            torch.save(data[name],f'processed_data/{name}.pkl')
        else:
            print(f"Load {name}......")
            data[name] = torch.load(f'processed_data/{name}.pkl')
    ...
    for name in ['dev','test']:
        ...
        if not os.path.isfile(f'processed_data/{name}_test.pkl'):
            print(f"Start {name}......")
            with open(f"data/{name}.json") as f:
                file = json.load(f)
                file = [data for data in file['data']]
            ...
            torch.save(data[name],f'processed_data/{name}_test.pkl')
        else:
            print(f"Load {name}......")
            data[name] = torch.load(f'processed_data/{name}_test.pkl')
    ...
    # +2 For CLS and SEP
    model = Answer(input_size=768,answer_size=pre.ctx_max_len+2).to(device)
    if os.path.isfile("ckpt/best_bert_475.ckpt"):
        print("Load models........")
        model.linear_start.load_state_dict(torch.load("ckpt/best_linear_start_475.ckpt",map_location= device))
        model.linear_end.load_state_dict(torch.load("ckpt/best_linear_end_475.ckpt",map_location= device))
        model.linear_answerable.load_state_dict(torch.load("ckpt/best_class_475.ckpt",map_location= device))
        bert.load_state_dict(torch.load("ckpt/best_bert_475.ckpt",map_location= device))
        solver = Solver(device,pre.tokenizer)
        #print(bert)
